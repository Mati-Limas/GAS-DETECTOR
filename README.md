# GAS-DETECTOR

<p align="center">
  <img alt="Main language" src="https://img.shields.io/badge/C%2B%2B-ESP32-blue?style=for-the-badge&logo=cplusplus">
  <img alt="Arduino" src="https://img.shields.io/badge/Arduino-Compatible-00979D?style=for-the-badge&logo=arduino&logoColor=white">
  <img alt="Wokwi" src="https://img.shields.io/badge/Simulated%20on-Wokwi-purple?style=for-the-badge">
  <img alt="Status" src="https://img.shields.io/badge/status-in%20development-yellow?style=for-the-badge">
</p>

<p align="center">
  <strong>Choose your language / Escolha seu idioma</strong>
</p>

<p align="center">
  <a href="./README_pt.md">Portugues</a>
  &nbsp;|&nbsp;
  <a href="./README_en.md">English</a>
</p>

---

## Project Overview

**GAS-DETECTOR** is a cyber-physical system prototype for flammable gas detection using ESP32, C++ and Wokwi simulation.

For the complete documentation, select one of the README versions above.

## Quick Links

- [Documentacao em Portugues](./README_pt.md)
- [Documentation in English](./README_en.md)
- [Wokwi Simulation](https://wokwi.com/projects/462937229989048321)
